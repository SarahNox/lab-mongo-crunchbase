      ...
      rl.question('Type an option: ', (option) => {
        switch(option){
          ...
          case "3":
            ...
            // not working
          case "4":
            db.collection('companies').count({"founded_year": 2004}, {"founded_month": 2}, {name: 1, _id:0}).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
            ...
            break;
          case "5":
            db.collection('companies').find({ "founded_year":2004, $and: [ {"founded_month":{$gte :4}}, {"founded_month": {$lte :6}}]}
          ...
          case "6":
            ...
            break;
          // case "7":
          //   db.collection('companies').({}).toArray((error, result) => {
          //     if (error) {
          //       console.log(error);
          //       rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
          //     } else {
          ...
          case "12":
            ...
            break;
          case "13":
            db.collection('companies').find({"relationships.person.permalink" :"david-ebersman"}, {"relationships" :1}).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
            ...
            break;
          case "14":
            db.collection('companies').find({name: "Facebook"}, {"competitors" :1}).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
